...
const page = () => {
    ...
    const nextStep = async () => {
        ...
        if (!valid) return toast.error('Mohon lengkapi data terlebih dahulu')
        setStep((s) => s + 1)
    }
                ...
                <div className="container mx-auto">
                    <section className=" bg-white">
                        ...
                        <p className="text-center text-gray-600 max-w-4xl mx-auto mb-8 text-base md:text-xl leading-relaxed">
                            ...
                            Lengkapi data berikut agar petugas dapat menindaklanjuti laporan secara tepat dan efisien.
                        </p>
                    </section>
                            ...
                            <AnimatePresence mode="wait">
                                ...
                                {step === 1 && (
                                    <motion.div key="step1" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                                        <h3 className="text-xl font-semibold mb-4 text-[#007E5B]">Informasi Pelapor</h3>
                                        <input {...register('nama', { required: true })} placeholder="Nama Lengkap" className="input mb-4" />
                                        <input {...register('email', { required: true })} placeholder="Email" className="input mb-4" />
                                        <button onClick={nextStep} className="btn-primary ml-auto">Lanjut <ArrowRight size={18} /></button>
                                    </motion.div>
                                )}

                                {step === 2 && (
                                    <motion.div key="step2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                        <h3 className="text-xl font-semibold mb-4 text-[#007E5B]">Jenis Masalah</h3>
                                        ...
                                        <textarea {...register('deskripsi', { required: true })} placeholder="Deskripsi Masalah" className="input mb-4 h-28" />
                                        <div className="flex justify-between">
                                            <button onClick={prevStep} className="btn-secondary"><ArrowLeft size={18} /> Kembali</button>
                                            <button onClick={nextStep} className="btn-primary">Lanjut <ArrowRight size={18} /></button>
                                        </div>
                                    </motion.div>
                                )}
                                ...
                                {step === 3 && (
                                    <motion.div key="step3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                        <h3 className="text-xl font-semibold mb-4 text-[#007E5B]">Upload Bukti</h3>
                                        <input type="file" {...register('bukti', { required: true })} className="mb-6 text-neutral-400 border-2 border-neutral-400 hover:bg-[#007E5B] hover:text-white px-6 py-2 rounded-lg cursor-pointer" />
                                        <div className="flex justify-between">
                                            <button onClick={prevStep} className="btn-secondary"><ArrowLeft size={18} /> Kembali</button>
                                            <button onClick={handleSubmit(onSubmit)} className="btn-primary">Kirim Laporan</button>
                                        </div>
                                    </motion.div>
                                )}
                                ...
                                {step === 4 && (
                                    <motion.div key="success" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
                                        <div className="text-center py-10">
                                            ...
                                            <h3 className="text-2xl font-bold mb-2 text-[#007E5B]">Laporan Terkirim</h3>
                                            <p className="text-gray-600">Terima kasih telah berkontribusi menjaga lingkungan.</p>
                                        </div>
                                    </motion.div>
                                )}
